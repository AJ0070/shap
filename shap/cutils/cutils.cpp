...
void compute_grey_code_row_values(
    ...
    const nb::ndarray<uint32_t, nb::shape<-1>, nb::device::cpu>& inds,
    nb::ndarray<double, nb::shape<-1, -1>, nb::device::cpu>& outputs,
    const nb::ndarray<double, nb::shape<-1>, nb::device::cpu>& shapley_coeff,
    const nb::ndarray<uint32_t, nb::shape<-1>, nb::device::cpu>& extended_delta_indexes,
    const int noop_code
) {
	...
	for (size_t i=0; i<pow(2, M); i++) {
		...
		for (size_t rvi = 0; rvi < rv.shape(0); rvi++) {
			...
			for (size_t rvj = 0; rvj < rv.shape(1); rvj++) {
				...
				// std::cout << "i: " << i << "rvi: " << rvi << "rvj: " << rvj << " Row value update: " << multiplication_factor * outputs(i, rvj) << std::endl;
			}
		}
        }
		// update the output row values
...
NB_MODULE(cutils, m) {
    m.def("compute_grey_code_row_values", &compute_grey_code_row_values);
}
